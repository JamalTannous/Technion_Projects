...
void RPLNode::sendDIOMessage(RPLMessage * dioMessage){
	...
	if (dioMessage -> sourceMAC != parent-> nodeMAC){
		...
		RPLMessage* messageToSend = new RPLMessage (dioMessage->messageID,dioMessage->messageType, dioMessage->sourceIP, dioMessage->destinationIP, nodeMAC, parent->nodeMAC);
	...
	for (int i = 0; i < children.size(); i++){
		if (dioMessage -> sourceMAC != children[i]-> nodeMAC){
			RPLMessage* messageToSend = new RPLMessage (dioMessage->messageID,dioMessage->messageType, dioMessage->sourceIP, dioMessage->destinationIP, nodeMAC, children[i]->nodeMAC);
...
double RPLNode::calculateRatioBetweenDIOAndRegularMessages() {
    return (tcpMessagesSent + tcpAckMessagesSent) / DIOMessagesSent;
}